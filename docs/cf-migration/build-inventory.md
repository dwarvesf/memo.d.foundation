...
public/content/*.md + assets + public/content/db/{vault,processing_metadata*}.parquet
   ...
   |   `mix duckdb.export`. It is NOT run by `make build-static` or `pnpm run build`.
   |   SUPERSEDED TWICE: it used to run only on a manual dispatch.yml trigger, which is
   |   why the parquet went stale; the task itself was then ported to TypeScript as
   |   scripts/duckdb-export.ts (`make duckdb-export`), and scripts/build-and-deploy.sh
   |   now runs that on every publish. dispatch.yml is deleted.)
   v
...
| ---------------------------------------- | -------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------- |
| Vault DB (rows + both embedding columns) | `db/vault.parquet` (also copied to `public/content/db/vault.parquet`)                  | git-committed binary; refreshed by `make duckdb-export` (`scripts/duckdb-export.ts`, the TypeScript port of the Elixir task), run as the reindex stage of `scripts/build-and-deploy.sh` on every publish (was manual-only via the deleted `dispatch.yml`) | Yes, every `generate-*.ts` script reads it directly                           |
...
| --------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `vault.parquet` (rows + metadata)                         | Yes, git-committed, structurally correct                                                                                                                                                                                                              | **Freshness/automation gap**: only regenerated by a manually-triggered Elixir GitHub Action (`dispatch.yml`), last run 2025-10-22, ~9 months stale relative to current `main` (086b7a2, 2026-07-19). N.4/N.5 either uploads the stale committed file as-is, or must also trigger/automate `mix duckdb.export` (which still needs Elixir + `GEMINI_API_KEY`/`JINA_API_KEY`/`INFINITY_API_KEY`) before upload. **CLOSED**: the reindex is now a stage of `scripts/build-and-deploy.sh`, so the parquet is refreshed on every publish and uploaded to R2 from there. It no longer needs Elixir either: the task was ported to `scripts/duckdb-export.ts`, which carries embeddings forward unless `MEMO_EMBEDDINGS` opts in, so the paid embedding providers are off the publish path. `INFINITY_*` turned out to be dead: no code reads it. |
