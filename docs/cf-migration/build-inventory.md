...
public/content/*.md + assets + public/content/db/{vault,processing_metadata*}.parquet
   ...
   |   SUPERSEDED: it used to run only on a manual dispatch.yml trigger, which is why
   |   the parquet went stale; scripts/build-and-deploy.sh now runs it on every
   |   publish, and dispatch.yml is deleted.)
   v
...
| ---------------------------------------- | -------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------- |
| Vault DB (rows + both embedding columns) | `db/vault.parquet` (also copied to `public/content/db/vault.parquet`)                  | git-committed binary; refreshed by `mix duckdb.export`, run as the reindex stage of `scripts/build-and-deploy.sh` on every publish (was manual-only via the deleted `dispatch.yml`) | Yes, every `generate-*.ts` script reads it directly                           |
