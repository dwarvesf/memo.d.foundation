  ...
  try {
    ...
    if (fs.existsSync(parquetFilePath)) {
      await parquetRead({
        ...
        columns: ['file_path', 'title', 'description', 'md_content', 'spr_content', 'tags', 'authors', 'date', 'draft', 'hiring', 'status'],
        onComplete: (data) => {
          searchIndex = data.map((row, idx) => {
          ...
          miniSearch = new MiniSearch({
            ...
            storeFields: ['file_path', 'title', 'description', 'md_content', 'spr_content', 'tags', 'authors', 'date', 'category'],
            searchOptions: {
 ...
 */
export function getSerializableSearchIndex(): { index: unknown; documents: Partial<Document>[] } | null {
  if (!miniSearch) {
    return null;
  }
