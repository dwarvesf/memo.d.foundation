 ...
 */
function nestPaths(data: { grouped_path: string; file_paths: string[] }[]): Record<string, GroupedPath> {
  ...
  data.forEach(item => {
    ...
    paths.forEach((path: string, index: number) => {
      ...
      if (index === paths.length - 1) {
        currentLevel[path].file_paths = item.file_paths.map((paths: string) => JSON.parse(paths));
 ...
 */
function sortGroupedPaths(obj: Record<string, GroupedPath>): Record<string, GroupedPath> {
  ...
  try {
    ...
    if (fs.existsSync(parquetFilePath)) {
      ...
      await parquetRead({
        ...
        columns: ['file_path', 'short_title', 'title', 'date', 'draft', 'hiring', 'status', 'hide_on_sidebar'],
        onComplete: (data) => {
          ...
          data.forEach(row => {
            ...
            if (!title || draft || hiring || hideOnSidebar || (status && status !== 'Open')) {
              return;
            }
            
            // Skip archived files and _radar files
            if (filePath.includes('archived/') || filePath.includes('_radar/')) {
              return;
            }
            ...
            if (groupedPath) {
              ...
              groupedPathMap.get(groupedPath)!.file_paths.push(JSON.stringify(filePathObj));
            }
          });
          
          // Convert map to array format expected by nestPaths
          parsedData = Array.from(groupedPathMap.entries()).map(([grouped_path, data]) => ({
            grouped_path,
 ...
 */
export async function getPinnedNotes(limit: number = 3): Promise<{ title: string; url: string; date: string }[]> {
      ...
      await parquetRead({
        ...
        onComplete: (data) => {
          ...
          pinnedNotes = filteredData.map(row => {
            ...
            const url = '/' + (slugifiedPath.endsWith('.md') ? 
              slugifiedPath.slice(0, -3) : slugifiedPath);
