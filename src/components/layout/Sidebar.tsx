...
const Sidebar = ({ isOpen, setIsOpen }: SidebarProps) => {
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <div
        className={`bg-background border-border fixed top-0 left-0 z-40 flex h-full w-[72px] flex-col border-r py-4 pb-12 font-sans transition-transform duration-300 ease-in-out md:w-[56px] lg:w-[var(--nav-sidebar-width)] ${isOpen ? 'translate-x-0' : 'translate-x-[-100%] md:translate-x-0'} `}
        onClick={e => e.target === e.currentTarget && handleClickOutside()}
      >
        {/* Logo and title */}
        <Link
          href="/"
          className="mx-4 flex h-10 items-center justify-center gap-2 px-2 md:justify-start md:px-4"
        >
          <svg
          ...
          <span className="hidden font-sans text-xs leading-tight font-bold tracking-tight uppercase md:inline-block">
            Dwarves
            <br />
            Memo
          </span>
        ...
        <nav className="flex flex-1 flex-col p-4">
          {navLinks.map((item, index) => (
            <div className="mb-2" key={index}>
              <Link
                href={item.url}
                className={`hover:bg-muted dark:hover:bg-muted flex items-center justify-center rounded-lg text-sm font-medium transition-colors md:justify-start ${isActiveUrl(item.url) ? 'text-primary' : 'text-foreground/80'}`}
      ...
      <div className="pointer-events-none fixed z-50">
        {navLinks.map((item, index) => (
          <div
            key={index}
            id={`tooltip-${index}`}
            className="bg-secondary text-secondary-foreground hidden rounded-md px-2.5 py-1.5 text-xs font-medium shadow-md"
            role="tooltip"
          >
            <span>{item.title}</span>
            <div
              className="arrow invisible absolute h-2 w-2 bg-inherit before:visible before:absolute before:h-2 before:w-2 before:rotate-45 before:bg-inherit before:content-['']"
              data-popper-arrow
            ></div>
          </div>
        ))}
