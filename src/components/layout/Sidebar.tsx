...
const navLinks = [
  ...
  { title: 'Consulting', url: '/consulting', icon: '/assets/img/consulting.svg' },
...
const Sidebar = ({ isOpen, setIsOpen }: SidebarProps) => {
  ...
  return (
    <>
      ...
      >
        {/* Logo and title */}
        <Link href="/" className="flex items-center justify-center md:justify-start px-2 md:px-4 h-10 mx-4 gap-2">
          <svg className="min-w-6 w-6 h-6 shrink-0" viewBox="0 0 19 20" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <nav className="flex flex-col flex-1 p-4">
          {navLinks.map((item, index) => (
            <div className="mb-2" key={index}>
              <Link
                href={item.url}
                className={`flex items-center justify-center md:justify-start rounded-lg text-sm font-medium transition-colors hover:bg-muted dark:hover:bg-muted
                  ${isActiveUrl(item.url) ? 'text-primary' : 'text-foreground/80'}`}
              ...
              >
                ...
                ></div>
                <span className="hidden md:inline-block ml-3">{item.title}</span>
              </Link>
            </div>
          ))}
        </nav>
        ...
        <div className="px-4 pt-2 border-t border-border mx-4">
          <div className="flex justify-center md:justify-start gap-3 mt-3">
            <a href="https://github.com/dwarvesf" target="_blank" rel="noopener noreferrer" className="text-muted-foreground hover:text-foreground transition-colors">
            ...
            <a href="https://discord.gg/dwarvesv" target="_blank" rel="noopener noreferrer" className="text-muted-foreground hover:text-foreground transition-colors">
              <svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <a href="https://www.facebook.com/dwarvesf" target="_blank" rel="noopener noreferrer" className="text-muted-foreground hover:text-foreground transition-colors">
              <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24">
                <path fill="currentColor"
                  d="M22 12c0-5.52-4.48-10-10-10S2 6.48 2 12c0 4.84 3.44 8.87 8 9.8V15H8v-3h2V9.5C10 7.57 11.57 6 13.5 6H16v3h-2c-.55 0-1 .45-1 1v2h3v3h-3v6.95c5.05-.5 9-4.76 9-9.95" />
              </svg>
            </a>
            <a href="https://dwarves.foundation/" target="_blank" rel="noopener noreferrer" className="text-muted-foreground hover:text-foreground transition-colors">
              <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24">
                <path fill="currentColor"
            ...
            <a href="mailto:team@dwarves.foundation" target="_blank" rel="noopener noreferrer" className="text-muted-foreground hover:text-foreground transition-colors">
      ...
      {/* Tooltips container (only visible on desktop when sidebar is collapsed) */}
      <div className="fixed z-50 pointer-events-none">
        {navLinks.map((item, index) => (
          <div 
            key={index} 
            id={`tooltip-${index}`} 
            className="hidden bg-secondary text-secondary-foreground font-medium py-1.5 px-2.5 text-xs rounded-md shadow-md"
            role="tooltip"
          >
            <span>{item.title}</span>
            <div className="arrow absolute w-2 h-2 bg-inherit invisible before:visible before:absolute before:w-2 before:h-2 before:bg-inherit before:content-[''] before:rotate-45" data-popper-arrow></div>
          </div>
        ))}
      </div>
    </>
