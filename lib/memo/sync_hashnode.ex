defmodule Memo.SyncHashnode do
  ...
  alias Memo.Common.{FileUtils, Frontmatter}
  require Logger
  ...
  defp process_file(file) do
    with {:ok, content} <- File.read(file),
      ...
      if is_nil(hashnode_meta) do
        publish_post(file, frontmatter, content_without_frontmatter)
      else
        update_post(file, frontmatter, content_without_frontmatter, hashnode_meta)
      end
    else
      _ ->
