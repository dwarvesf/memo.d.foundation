...
def alter_columns(conn, frontmatter):
	...
	for key, value in frontmatter.items():
		...
		elif "embeddings_spr_mxbai" in key:
...
def regenerate_embeddings(md_content, frontmatter):
	# Generate an SPR for the md_content and embed it
	spr_content = spr_compress(md_content)
	print(f"SPR: {spr_content}")
	spr_content_text = spr_content.choices[0].message.content if spr_content else None
	frontmatter["spr_content"] = spr_content_text

	# Generate embeddings for the SPR
	embeddings_spr_mxbai = embed_mxbai(spr_content_text)
	print(f"SPR Embeddings: {embeddings_spr_mxbai}")
	embeddings_spr_mxbai_values = (
		embeddings_spr_mxbai.data[0].embedding if embeddings_spr_mxbai else None
	)
	frontmatter["embeddings_spr_mxbai"] = embeddings_spr_mxbai_values
...
def setup_frontmatter(md_content, frontmatter, short_path):
...
def add_to_database(conn, file_path, frontmatter, md_content):
	...
	setup_frontmatter(md_content, frontmatter, short_path)
	...
	if existing_row:
		...
		row_last_updated = existing_row["last_updated"][0].as_py()

		# Check if the SPR content and openai embeddings exist and update them
		row_spr_content = (
			existing_row["spr_content"][0].as_py()
			if existing_row["spr_content"]
			else None
		)
		row_embeddings_spr_mxbai = (
			existing_row["embeddings_spr_mxbai"][0].as_py()
			if existing_row["embeddings_spr_mxbai"]
			else None
		)
		row_md_content = existing_row["md_content"][0].as_py()

		if (
			not row_spr_content
			or not row_embeddings_spr_mxbai
			or not row_md_content
